...
public class ShortestPaths {
	...
	public void run() {
		...
		//
		while (!pq.isEmpty()) { //can only work when MinHeap is not empty
			VertexAndDist minValue = pq.extractMin();
			for (Edge e : minValue.getVertex().edgesFrom()){
				Decreaser<VertexAndDist> d = map.get(e.to);
				int a = weights.get(e);
				int currentDist = d.getValue().getDistance();
				int startingValue = map.get(minValue).getValue().getDistance();
				
				if ((startingValue + a) < currentDist){
					d.decrease(d.getValue().sameVertexNewDistance(startingValue + a));
	...
	public LinkedList<Edge> returnPath(Vertex endVertex) {
		...
		// FIXME
		//
		Vertex currV = endVertex;
		while (!startVertex.equals(currV)) {
			path.addFirst(toEdge.get(currV));
			currV = toEdge.get(currV).from;
		}
		return path;
	}
